...
const EditParticipant: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (participant) {
      setFormData({ 
        ...participant,
        signedUpYear: participant.signedUpYear || participant.signUpYear || new Date().getFullYear().toString(),
        introductoryVenue: participant.introductoryVenue || participant.introVenue || "",
      });
    } else {
      setNotFound(true);
    }
  ...
  // Auto-calculate age
  useEffect(() => {
    if (formData && formData.dateOfBirth) {
      ...
      if (formData.age !== age) {
        setFormData(prev => prev ? ({ ...prev, age }) : null);
      }
    }
  ...
  const validate = () => {
    ...
    if (!formData.participantName) newErrors.participantName = "Participant Name is required";
    if (!formData.program) newErrors.program = "Program is required";
    if (!formData.participantNumber) newErrors.participantNumber = "Contact Number is required";
    if (!formData.participantEmail) newErrors.participantEmail = "Email is required";
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedParticipant: Participant = {
      ...
      signUpYear: formData.signedUpYear,
      introVenue: formData.introductoryVenue,
      intake: formData.intakeConfirmation || formData.intakeBasedOnRegistration || formData.intake,
    };
    ...
    const index = localParticipants.findIndex((lp) => lp.id === updatedParticipant.id);
    if (index !== -1) {
      localParticipants[index] = updatedParticipant;
    } else {
      localParticipants.push(updatedParticipant);
    }

    localStorage.setItem("livesmart_participants", JSON.stringify(localParticipants));
  ...
  if (notFound) {
    return (
      <div className="container py-5 text-center">
        ...
        <Link to="/participants" className="btn btn-primary mt-3">Back to Participants</Link>
      </div>
    );
  }
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 className="fw-bold mb-0">Edit Participant V2</h2>
          <p className="text-muted">Update details for {formData.participantName}</p>
        </div>
        <div className="d-flex gap-2">
          <Link to="/participants" className="btn btn-outline-secondary">Cancel</Link>
          <button type="submit" form="participantEditForm" className="btn btn-primary px-4">Update Participant</button>
        </div>
      </div>

      <form id="participantEditForm" onSubmit={handleSubmit} className="row g-4">
            ...
            <div className="card-body">
              <div className="row g-3">
                ...
                <div className="col-md-4">
                  ...
                  <input type="text" name="tShirtSize" className="form-control" value={formData.tShirtSize} onChange={handleChange} />
                </div>
          ...
          <div className="card border-0 shadow-sm mb-4">
            ...
            <div className="card-body">
              <div className="row g-3">
                ...
                <div className="col-12">
                  <label className="form-label small fw-bold text-muted">Balance</label>
              ...
              <div className="mt-4">
                <button type="submit" className="btn btn-primary w-100 py-2 fw-bold shadow-sm">Update Participant</button>
              </div>
            </div>
          </div>
        </div>
